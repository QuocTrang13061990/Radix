...
if (isPost()) {
    ...
    if (empty($errors)) {
        ...
        $editStatus = editData('blogs', $dataEdit, "id=$blogId");
        if ($editStatus) {
        ...
        redirect('admin?modules=blogs&action=edit&id='.$blogId);
    }
}
// Truy vấn lất tất cả danh mục để hiển thị option
$listAllCate = getRaw("SELECT id, name FROM blog_categories ORDER BY name");
// echo '<pre>';
// print_r($listAllCate);
// echo '</pre>';
// exit;
...
$oldData = getFlashData('blogDetail');
